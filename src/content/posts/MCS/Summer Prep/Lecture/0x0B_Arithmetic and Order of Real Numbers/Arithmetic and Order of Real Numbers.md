---
title: "Arithmetic and Order of Real Numbers"
...
- **Definition 2**  
  ...
  - **Proposition 2.4**  
    ...
    - **Proof 2.4.1**  
      ($\Longrightarrow$) Suppose $a>0$. By the axiom of the multiplicative inverse, there exists $a^{-1}=\frac{1}{a}$ such that $a\cdot \frac{1}{a}=1$. Since $1>0$ and $a>0$, it follows from the properties of order and multiplication that $\frac{1}{a}$ must be positive, as a positive number multiplied by a negative number would yield a negative result. Thus, $\frac{1}{a}>0$.

      ($\Longleftarrow$) Suppose $a\in\mathbb R$ such that $\frac{1}{a}>0$. Applying the forward implication ($\Longrightarrow$) to the positive real number $\frac{1}{a}$, we have that its multiplicative inverse must also be positive, that is, $\left(\frac{1}{a}\right)^{-1}>0$. Since $a \cdot \frac{1}{a} = 1$, multiplying both sides by $\left(\frac{1}{a}\right)^{-1}$ yields $a = \left(\frac{1}{a}\right)^{-1}$. Therefore, we have $a>0$.
