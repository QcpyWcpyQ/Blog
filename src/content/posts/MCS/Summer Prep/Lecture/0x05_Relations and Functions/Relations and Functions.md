...
- **Remark 8**  
  ...
  - **Example 8.2**  
    ...
    \end{aligned}
    $$
    Note that $g\circ f\colon \mathbb R\to \mathbb R$ and $f\circ g\colon \mathbb R\to \mathbb R$ with
    $$
    \begin{aligned}
...
- **Definition 11**  
  A function $f\colon X\to Y$ is called **invertible** if there exists a function $g\colon Y\to X$ such that $g\circ f=\text{id}_X$ and $f\circ g=\text{id}_Y$. Such a function is called **inverse** of $f$.

- **Remark 11**  
  If $f\colon X\to Y$ is invertible then its inverse is unique. In fact, suppose that $g,g^\prime\colon Y\to X$ are both inverses of $f$, then
  $$
  ...
  - **Theorem 11.2**  
    ...
    - **Proof 11.2.1**  
      ($\Longrightarrow$) Suppose that $f$ is invertible then there exists $f^{-1}\colon Y\to X$ such that $f^{-1}\circ f=\text{id}_X$ and $f\circ f^{-1}=\text{id}_Y$.
